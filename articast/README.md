...
```bash
articast$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
articast$ sam local start-api
articast$ curl http://localhost:3000/
```
...
```bash
articast$ sam logs -n HelloWorldFunction --stack-name articast --tail
```
...
```bash
sam delete --stack-name articast
```
